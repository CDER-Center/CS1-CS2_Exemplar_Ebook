...
This is material for teaching fork/join programming in a CS2 class, as taught by David Bunde at knox College.
They are heavily influenced by the work of Dan Grossman [1].
...
The slides are in 1 deck; for class, David would add review and homework discussion slides to the beginning of each day, but these are removed from the shared deck since they’ll vary by course and won’t fall in the same place (David adds them each day based on where he gets in the previous lecture).
The comments on the slides flesh out what David would say and explain the answers to the clicker questions.

Also related to these slides are the following:
* Recorded lectures of David presenting these slides over two class sessions: [lecture1](https://youtu.be/LO2cbi-M7jI) and [lecture2](https://youtu.be/pQ3PQY4viXA).
He is using slightly older slides so the content doesn't exactly match the current slides.
...
Prime testing is just checking 3 to sqrt(n) as possible divisors (only check odd numbers).  David has used this in class and feels that it works pretty well.
* Materials for Lab 9 ([lab9.tex](lab9.tex), [lab9.pdf](lab9.pdf), [lab9given.zip](lab9given.zip)), which asks the students to change the code for summing an array in parallel to implement some other reductions.  David has only given it once.  Students struggled to implement finding the kth smallest value in this framework.  He has plans to revise the lab to address the issue.  This lab is less interesting/important than Lab 8 so skip it if you’re only doing one.  (Alternatively, they could be combined if your labs are more substantial and are submitted for a grade.)
* A possible replacement lab ([heatfork.tex](heatfork.tex), [heatfork.pdf](heatfork.pdf), [heatSlides.pptx](heatSlides.ppts), [heatDiffusionLab.zip](heatDiffusionLab.zip)). This asks the students to parallelize the heat diffusion simulator using fork and join.  David hasn’t used it in this form, but has used the lab with Java threads and its Executor framework.  The slides are used in lecture the day before the lab to introduce the application (also include a demo).
