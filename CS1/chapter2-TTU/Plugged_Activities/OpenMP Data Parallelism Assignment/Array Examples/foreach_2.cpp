/*
	Title:  foreach_array.cpp
	Author: 
	Description: Using a foreach loop with an array
	...
	COMPILE:  g++ foreach_array.cpp -std=c++11
...
int main() 
{
	string words[] = {"Dog","Cat","Horse","Cow","Chicken","Rat","Pig","Iguana","Spider"};
	int count;

	cout << endl << "Printing the words in the array after prepending the count of the number of characters to each word using a foreach loop.\n";
	for(auto word : words)
	{
		count = 0;
	...
	cout << "Now printing the words in the array again using another foreach loop.\n";
	for(auto str : words)
		cout << str << endl;
